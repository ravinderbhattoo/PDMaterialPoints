...
#     radius = 0.2 + 1.0*rand()
#     obj = changetype(obj, out -> begin x=out[:x]; sum((x .- vec(center)).^2, dims=1) .< radius^2 end, 4)
#     obj = changetype(obj, out -> begin x=out[:x]; sum((x .- vec(center)).^2, dims=1) .< (radius - 0.1)^2 end, 5)
# end

# obj = delete!(obj, out -> begin x=out[:x]; sum(x[1:2, :].^2, dims=1) .> 4.0^2 end)
